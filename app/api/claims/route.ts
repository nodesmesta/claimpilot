...
function parseClaimPDF(text: string): Record<string, unknown> {
  ...
  return {
    claim_id: get(/Claim ID:\s*(.+)/i) || `CLM-${Date.now()}`,
    policyholder: get(/Policyholder:\s*(.+)/i) || get(/Name:\s*(.+)/i) || "Unknown",
    policy_type: get(/Policy:\s*(.+)/i) || get(/Policy Type:\s*(.+)/i),
    incident_type: get(/Incident Type:\s*(.+)/i) || get(/incident[^:]*:\s*(.+)/i),
